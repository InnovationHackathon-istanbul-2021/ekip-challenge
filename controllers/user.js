...
const getAdminProfile = async (req, res) => {
    ...
    const currentAdmin = await userModel.findOne({ _id: id });

    console.log('here in user controller', currentAdmin);
    if (currentAdmin) {
        // create an array for each orders and fetch each order 
        const ordersWithProducts = [];

        for (let i = 0; i < currentAdmin.orders.length; i++) {
            const newOrder = {};
            const order = await orderModel.findOne({ _id: currentAdmin.orders[i] });
            newOrder.totalPrice = order.totalPrice;
            newOrder.items = [];
            for (let j = 0; j < order.items.length; j++) {
                const product = await productModel.findOne({ _id: order.items[j].productRef });
        ...
        // using this array for each order's item
        res.render('/orders', { admin: currentAdmin, recentOrders: ordersWithProducts }); // TODO pass this to render
    } else {
        res.status(404).json('user not found');
    }
};
